                ...
                <div class="flex items-center justify-between flex-wrap gap-4">
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <h2 class="text-xl font-bold text-gray-900 truncate">{{ $fileRequest->title }}</h2>
                            @if($fileRequest->description)
                                <p class="text-sm font-medium text-gray-600 mt-1">{{ $fileRequest->description }}</p>
                            @endif
            ...
            <div class="p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-gradient-to-br from-indigo-50/50 to-purple-50/30 rounded-xl border-2 border-indigo-200 p-5">
                        ...
                        <dd class="text-base font-bold text-gray-900">{{ $fileRequest->client->name }}</dd>
                        @if($fileRequest->client->email)
                            <dd class="text-sm font-medium text-indigo-600 mt-1">
                                <a href="mailto:{{ $fileRequest->client->email }}" class="hover:underline">{{ $fileRequest->client->email }}</a>
                            </dd>
                        @endif
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-50/50 to-pink-50/30 rounded-xl border-2 border-purple-200 p-5">
                        ...
                        <dd class="text-base font-bold text-gray-900">{{ $fileRequest->category->name ?? '—' }}</dd>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-pink-50/50 to-violet-50/30 rounded-xl border-2 border-pink-200 p-5">
                        ...
                        <dd class="text-base font-bold text-gray-900">{{ $fileRequest->financial_year ?? '—' }}</dd>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-violet-50/50 to-indigo-50/30 rounded-xl border-2 border-violet-200 p-5">
                        ...
                        <dd class="text-base font-bold text-gray-900">{{ $fileRequest->requester->name }}</dd>
                        <dd class="text-sm font-medium text-gray-600 mt-1">{{ $fileRequest->created_at->format('M d, Y H:i') }}</dd>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-emerald-50/50 to-teal-50/30 rounded-xl border-2 border-emerald-200 p-5">
                        ...
                        @if($fileRequest->expires_at && $fileRequest->expires_at->isFuture())
                            <dd class="text-sm font-medium text-emerald-600 mt-1">{{ $fileRequest->expires_at->diffForHumans() }}</dd>
                        @elseif($fileRequest->expires_at && $fileRequest->expires_at->isPast())
                    ...
                    <div class="mt-8 pt-8 border-t-2 border-indigo-200">
                        ...
                        <div class="bg-gradient-to-br from-indigo-50/50 to-purple-50/30 rounded-xl border-2 border-indigo-200 p-6">
                            ...
                            <div class="flex items-center gap-3">
                                <input type="text" 
                                       readonly 
                                       value="{{ route('file-requests.upload', $fileRequest->token) }}" 
                                       id="upload-link"
                            ...
                            <p class="mt-3 text-xs font-medium text-gray-600 flex items-center">
                                ...
                                This link will expire on {{ $fileRequest->expires_at->format('M d, Y') }} ({{ $fileRequest->expires_at->diffForHumans() }})
                            </p>
                        </div>
                    </div>
                @endif
