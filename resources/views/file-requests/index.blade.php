<x-app-layout>
    ...
    <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-2xl border-2 border-indigo-200/60 p-6 hover:border-indigo-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mt-1">{{ $fileRequests->total() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 rounded-2xl border-2 border-amber-200/60 p-6 hover:border-amber-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent mt-1">{{ $fileRequests->where('status', 'pending')->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 rounded-2xl border-2 border-emerald-200/60 p-6 hover:border-emerald-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent mt-1">{{ $fileRequests->where('status', 'completed')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-white via-indigo-50/30 to-purple-50/30 rounded-2xl border-2 border-indigo-200/60 shadow-xl p-6 backdrop-blur-sm">
            <form method="GET" action="{{ route('file-requests.index') }}" class="space-y-4">
                ...
                <div class="flex items-center justify-between pt-4 border-t-2 border-indigo-200">
                    <div class="flex gap-3">
                        ...
                        @if(request('status') || request('client_id'))
                            <a href="{{ route('file-requests.index') }}" class="inline-flex items-center px-5 py-2.5 bg-white border-2 border-gray-300 rounded-xl font-semibold text-sm text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 shadow-sm hover:shadow-md">
                                <svg class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($fileRequests as $fileRequest)
                            <tr class="hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-purple-50/30 transition-all duration-150 {{ $loop->even ? 'bg-gray-50/30' : 'bg-white' }} cursor-pointer group" onclick="window.location.href='{{ route('file-requests.show', $fileRequest) }}'">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900 group-hover:text-indigo-700 transition-colors">{{ $fileRequest->title }}</div>
                                    @if($fileRequest->description)
                                        <div class="text-xs font-medium text-gray-600 mt-1">{{ Str::limit($fileRequest->description, 50) }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-800">{{ $fileRequest->client->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-800">{{ $fileRequest->requester->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap" onclick="event.stopPropagation()">
                                    ...
                                    @elseif($fileRequest->status === 'expired')
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-800">
                                        {{ $fileRequest->expires_at ? $fileRequest->expires_at->format('M d, Y') : '—' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                                    <x-table-action-dropdown :item="$fileRequest">
                                        <a href="{{ route('file-requests.show', $fileRequest) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 hover:text-blue-700 transition-colors">View</a>
                                        @can('files.delete')
                                            <form method="POST" action="{{ route('file-requests.destroy', $fileRequest) }}" 
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @can('files.upload')
                                            <div class="mt-6">
                                                <a href="{{ route('file-requests.create') }}">
