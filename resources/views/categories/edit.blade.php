<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-300">Update category information</p>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-gradient-to-br from-white via-indigo-50/30 to-purple-50/30 rounded-2xl border-2 border-indigo-200/60 shadow-xl backdrop-blur-sm overflow-hidden">
            <form method="POST" action="{{ route('categories.update', $category) }}" x-data="{ submitting: false }" @submit="submitting = true">
                @csrf
                @method('PATCH')
                            ...
                            @can('files.retention.manage')
                                ...
                                <div>
                                    ...
                                    <input id="retention_days" 
                                           type="number" 
                                           name="retention_days" 
                                           value="{{ old('retention_days', $category->retention_days) }}" 
                                           min="0" 
