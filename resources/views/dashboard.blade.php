<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <a href="{{ route('clients.index') }}" class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-2xl border-2 border-indigo-200/60 p-6 hover:border-indigo-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
            ...
            <a href="{{ route('files.index') }}" class="bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 rounded-2xl border-2 border-blue-200/60 p-6 hover:border-blue-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent mt-1">{{ $filesThisMonth }}</p>
            ...
            <div class="bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 rounded-2xl border-2 border-emerald-200/60 p-6 hover:border-emerald-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent mt-1">{{ number_format($storageUsage / 1024 / 1024, 2) }} MB</p>
                        <p class="text-xs text-gray-500 mt-1">{{ number_format($storageUsage / 1024 / 1024 / 1024, 2) }} GB total</p>
                    </div>
                </div>
            </div>

            <a href="{{ route('audit-logs.index') }}" class="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 rounded-2xl border-2 border-amber-200/60 p-6 hover:border-amber-300 hover:shadow-xl transition-all duration-200 group transform hover:-translate-y-1">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <p class="text-3xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent mt-1">{{ $recentActivity->count() }}</p>
        ...
        <div class="bg-gradient-to-br from-white via-indigo-50/30 to-purple-50/30 rounded-2xl border-2 border-indigo-200/60 shadow-xl backdrop-blur-sm overflow-hidden">
            <div class="px-6 py-5 border-b-2 border-indigo-200/60 bg-gradient-to-r from-indigo-50/80 via-purple-50/80 to-pink-50/80">
                <div class="flex items-center justify-between">
                    ...
                    @if($recentActivity->count() > 0)
                        <a href="{{ route('audit-logs.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-indigo-700 bg-indigo-100 hover:bg-indigo-200 rounded-lg transition-colors duration-200">
                            View All
            ...
            <div class="p-6">
                @forelse($recentActivity as $activity)
                    <div class="flex items-start space-x-4 py-4 {{ !$loop->last ? 'border-b border-gray-200/60' : '' }} hover:bg-white/50 rounded-lg px-3 -mx-3 transition-colors duration-200">
                        <div class="flex-shrink-0 mt-1">
                            <div class="h-3 w-3 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 ring-4 ring-indigo-100"></div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-900 leading-relaxed">{{ $activity->description }}</p>
