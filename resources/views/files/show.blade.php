<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-300">{{ $file->original_name }}</p>
            </div>
            <div class="flex items-center gap-3">
                @can('files.preview')
                    <button onclick="previewFile({{ $file->id }})" class="inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-200 rounded-xl font-semibold text-sm text-blue-700 hover:from-blue-100 hover:to-cyan-100 hover:border-blue-300 transition-all duration-200 shadow-sm hover:shadow-md">
    ...
    <div class="max-w-6xl mx-auto space-y-6">
                    ...
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                @can('files.status.change')
                                    <form method="POST" action="{{ route('files.update-status', $file) }}" class="inline" onchange="this.submit()">
                                        @csrf
                                        @method('PUT')
                                        <select name="status" class="text-xs font-semibold rounded-lg border-2 border-purple-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-300 bg-white text-gray-900 px-2 py-1 transition-all duration-200">
                                            <option value="uploaded" {{ $file->status === 'uploaded' ? 'selected' : '' }}>Uploaded</option>
                                            <option value="pending_review" {{ $file->status === 'pending_review' ? 'selected' : '' }}>Pending Review</option>
                                            <option value="approved" {{ $file->status === 'approved' ? 'selected' : '' }}>Approved</option>
                                            <option value="needs_correction" {{ $file->status === 'needs_correction' ? 'selected' : '' }}>Needs Correction</option>
                    ...
                    @can('files.notes')
                        <button onclick="switchTab('notes')" 
                            ...
                            <span class="flex items-center">
                                ...
                                Notes <span id="notes-count" class="ml-1 px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-bold">{{ $file->notes->count() }}</span>
                            </span>
                        </button>
                    @endcan
                    @if($file->versions->count() > 0)
                        <button onclick="switchTab('versions')" 
                            ...
                            <span class="flex items-center">
                                ...
                                Versions <span class="ml-1 px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full text-xs font-bold">{{ $file->versions->count() }}</span>
                            </span>
                        </button>
                    @endif
                </nav>
                ...
                <div id="content-details" class="tab-content">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div class="bg-gradient-to-br from-indigo-50/50 to-purple-50/30 rounded-xl border-2 border-indigo-200 p-5">
                            ...
                            <dd class="text-sm font-bold text-gray-900 break-words">{{ $file->original_name }}</dd>
                        </div>
                        <div class="bg-gradient-to-br from-blue-50/50 to-cyan-50/30 rounded-xl border-2 border-blue-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">
                                <a href="{{ route('clients.show', $file->client) }}" class="text-blue-600 hover:text-blue-700 hover:underline">
                                    {{ $file->client->name }}
                                </a>
                            </dd>
                        </div>
                        <div class="bg-gradient-to-br from-purple-50/50 to-pink-50/30 rounded-xl border-2 border-purple-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ $file->category->name }}</dd>
                        ...
                        <div class="bg-gradient-to-br from-pink-50/50 to-rose-50/30 rounded-xl border-2 border-pink-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ $file->financial_year }}</dd>
                        </div>
                        <div class="bg-gradient-to-br from-emerald-50/50 to-green-50/30 rounded-xl border-2 border-emerald-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ $file->version }}</dd>
                        </div>
                        <div class="bg-gradient-to-br from-amber-50/50 to-orange-50/30 rounded-xl border-2 border-amber-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ number_format($file->size / 1024, 2) }} KB</dd>
                        </div>
                        <div class="bg-gradient-to-br from-cyan-50/50 to-blue-50/30 rounded-xl border-2 border-cyan-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ $file->uploader->name ?? 'System' }}</dd>
                        </div>
                        <div class="bg-gradient-to-br from-teal-50/50 to-emerald-50/30 rounded-xl border-2 border-teal-200 p-5">
                            ...
                            <dd class="text-sm font-semibold text-gray-900">{{ $file->created_at->format('M d, Y H:i') }}</dd>
                        </div>
                        @if($file->archived_at)
                            <div class="bg-gradient-to-br from-slate-50/50 to-gray-50/30 rounded-xl border-2 border-slate-200 p-5">
                                ...
                                <dd class="text-sm font-semibold text-gray-900">{{ $file->archived_at->format('M d, Y H:i') }}</dd>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="mt-8 pt-8 border-t-2 border-indigo-200 flex items-center gap-4 flex-wrap">
                        @can('files.lock')
                            @if(!$file->is_locked)
                                <form method="POST" action="{{ route('files.toggle-lock', $file) }}">
                                    @csrf
                    ...
                    <div id="content-notes" class="tab-content" style="display: none;">
                        <div class="space-y-6">
                            ...
                            <div id="notes-list" class="space-y-4">
                                @foreach($file->notes as $note)
                                    <div class="bg-white rounded-xl border-2 border-gray-200 p-5 shadow-md hover:shadow-lg transition-all duration-200">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <p class="text-sm font-medium text-gray-900 mb-2">{{ $note->note }}</p>
                                            ...
                                            @can('files.notes.delete')
                                                <form method="POST" action="{{ route('files.notes.destroy', $note) }}" onsubmit="return confirm('Are you sure you want to delete this note?');" class="ml-4">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @endforeach
                                <div id="notes-empty" class="text-center py-12 text-gray-500 text-sm {{ $file->notes->count() > 0 ? 'hidden' : '' }}">
                                ...
                                <div class="bg-gradient-to-r from-white to-purple-50/30 rounded-xl border-2 border-purple-200 p-6 hover:border-purple-300 transition-all duration-200 shadow-md hover:shadow-lg">
                                    <div class="flex items-center justify-between flex-wrap gap-4">
                                        <div class="flex items-center gap-4 flex-1 min-w-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="text-base font-bold text-gray-900 truncate">{{ $version->original_name }}</div>
                                                <div class="flex items-center gap-4 mt-2 text-xs text-gray-600 flex-wrap">
                                                    <span class="px-2.5 py-1 bg-purple-100 text-purple-700 rounded-lg font-bold">Version {{ $version->version }}</span>
                                                    <span class="flex items-center">
                                                    ...
                                                    <span class="flex items-center">
                                                        ...
                                                        {{ number_format($version->size / 1024, 2) }} KB
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3">
                                            @can('files.download')
                                                <a href="{{ route('files.download', $version) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 border border-transparent rounded-lg font-semibold text-sm text-white hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="flex items-center justify-end">
            <a href="{{ route('files.index') }}" class="inline-flex items-center px-6 py-3 bg-white border-2 border-gray-300 rounded-xl font-semibold text-sm text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 shadow-sm hover:shadow-md">
                <svg class="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <x-modal name="upload-version">
        <div class="p-6">
            ...
            <p class="text-sm text-gray-600 mb-6 text-center">Upload a new version of <span class="font-semibold text-gray-900">{{ $file->original_name }}</span></p>
            
            <form id="upload-version-form" method="POST" action="{{ route('files.upload-version', $file) }}" enctype="multipart/form-data">
                @csrf
                <div class="mb-6">
    ...
    <script>
        ...
        let notes = {!! json_encode($file->notes->map(fn($n) => ['id' => $n->id, 'note' => $n->note, 'user' => $n->user->name, 'created_at' => $n->created_at->format('M d, Y H:i'), 'created_at_human' => $n->created_at->diffForHumans()])) !!};
        ...
        async function addNote() {
            ...
            try {
                const response = await fetch('{{ route('files.notes.store', $file) }}', {
                    method: 'POST',
                    headers: {
